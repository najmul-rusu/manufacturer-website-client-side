...
const MyProfile = () => {
    ...
    // const { data: user, isLoading, refetch } = useQuery('user', () => fetch('https://shielded-retreat-18256.herokuapp.com/user', {
    //     method: 'GET',
    //     headers: {
    ...
    const { data: dbuser, isLoading, refetch } = useQuery(['user', user?.email], () => fetch(`https://shielded-retreat-18256.herokuapp.com/user/${user?.email}`).then(res => res.json()))
